...
import * as GContacts from "./lib/google-contacts.js";

// --- PDF generation helpers (html2canvas + jsPDF loaded on demand from CDN) ---
let _pdfLibsPromise = null;
function loadScriptOnce(src) {
  return new Promise((resolve, reject) => {
    if (document.querySelector(`script[data-ss-src="${src}"]`)) return resolve();
...
// Renders a DOM node (the invoice sheet) to a PDF Blob + a File, sized to the node's
// own dimensions (so Thermal80 receipts and A4 sheets each get an appropriately shaped page).
async function nodeToInvoicePdf(node, filename) {
  await ensurePdfLibs();
  ...
  const wMm = pxToMm(canvas.width / 2);
  const hMm = pxToMm(canvas.height / 2);
...
function GoogleContactsSection({ data }) {
  const [cfg, setCfg] = useState(() => GContacts.loadConfig());
  const [connected, setConnected] = useState(() => GContacts.isConnected());
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState("");
  const [progress, setProgress] = useState(null); // {done,total,errors}

  useEffect(() => {
    const unsub = GContacts.subscribe(() => {
      setConnected(GContacts.isConnected());
      setCfg(GContacts.loadConfig());
    });
    return unsub;
  }, []);

  const patients = safeArray(data.patients);
  const syncMap = GContacts.getSyncMap();
  const syncedCount = patients.filter(p => syncMap[p.id]).length;
  const unsyncedCount = patients.length - syncedCount;

  const updateCfg = (patch) => {
    const next = { ...cfg, ...patch };
    setCfg(next);
    GContacts.saveConfig(next);
  };

  const handleConnect = async () => {
    setBusy(true); setMsg("");
    try {
      await GContacts.connect({ interactive: true });
      setMsg("Connected to Google. You can now sync contacts.");
    } catch (e) {
      setMsg("Connect failed: " + e.message);
    } finally { setBusy(false); }
  };

  const handleDisconnect = () => {
    GContacts.disconnect();
    setMsg("Disconnected.");
  };

  const handleSyncAll = async () => {
    if (!confirm(`Sync ${patients.length} patient(s) to Google Contacts? This will create or update entries.`)) return;
    setBusy(true); setMsg(""); setProgress({ done: 0, total: patients.length, errors: [] });
    try {
      const res = await GContacts.syncMany(patients, (p) => setProgress({ ...p }));
      setMsg(`Done. ${res.ok}/${res.total} synced.${res.errors.length ? ` ${res.errors.length} failed.` : ""}`);
    } catch (e) {
  ...
  const handleSyncUnsynced = async () => {
    const map = GContacts.getSyncMap();
    const todo = patients.filter(p => !map[p.id]);
    if (!todo.length) { setMsg("All patients already synced."); return; }
    if (!confirm(`Sync ${todo.length} new patient(s) to Google Contacts?`)) return;
    setBusy(true); setMsg(""); setProgress({ done: 0, total: todo.length, errors: [] });
    try {
      const res = await GContacts.syncMany(todo, (p) => setProgress({ ...p }));
      setMsg(`Done. ${res.ok}/${res.total} synced.${res.errors.length ? ` ${res.errors.length} failed.` : ""}`);
    } finally { setBusy(false); }
  };

  const handleClearMap = () => {
    if (!confirm("Clear local sync record? This does NOT delete Google contacts but will cause re-sync to create duplicates.")) return;
    GContacts.clearSyncMap();
    setMsg("Local sync map cleared.");
  };

  const card = { background: "#fff", border: "1px solid #e8e2db", borderRadius: 12, padding: 20, marginBottom: 16 };
  const label = { fontWeight: 600, color: "#6b5e52", fontSize: 13, marginBottom: 4, display: "block" };
  const input = { width: "100%", padding: "9px 11px", border: "1.5px solid #e2ddd8", borderRadius: 8, fontSize: 14 };

  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
        <h2 style={{ margin: 0, fontFamily: "'Playfair Display',serif" }}>📇 Google Contacts Sync</h2>
        <span style={{
      ...
      <div style={card}>
        <div style={{ fontWeight: 700, marginBottom: 12 }}>1. Google OAuth Client ID</div>
        <div style={{ fontSize: 12, color: "#6b5e52", marginBottom: 10, lineHeight: 1.6 }}>
          Create one at <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noreferrer" style={{ color: "#1d4ed8" }}>console.cloud.google.com/apis/credentials</a> →
          <b> Create credentials → OAuth client ID → Web application</b>.
          Enable the <b>People API</b> in the same project. Add this site's origin
          (<code>{typeof window !== "undefined" ? window.location.origin : ""}</code>) to <b>Authorized JavaScript origins</b>.
        </div>
        ...
        <input
          ...
          value={cfg.clientId || ""}
          onChange={e => updateCfg({ clientId: e.target.value.trim() })}
        />
      </div>

      <div style={card}>
        <div style={{ fontWeight: 700, marginBottom: 12 }}>2. Connection</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {!connected
            ? <button className="btn" style={{ background: "#1a73e8", color: "#fff", border: "none", fontWeight: 700 }} disabled={busy || !cfg.clientId} onClick={handleConnect}>{busy ? "Connecting…" : "🔐 Connect to Google"}</button>
            : <button className="btn btn-outline" onClick={handleDisconnect}>Disconnect</button>
          }
        </div>
        <div style={{ fontSize: 11, color: "#9b8e82", marginTop: 8 }}>
          The access token lives in memory only and expires in ~1 hour. After that you'll be asked to reconnect.
        </div>
      </div>

      <div style={card}>
        <div style={{ fontWeight: 700, marginBottom: 12 }}>3. Auto-sync</div>
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
          <input type="checkbox" checked={!!cfg.autoSync} onChange={e => updateCfg({ autoSync: e.target.checked })} />
          <span>Automatically sync <b>new patient registrations</b> to Google Contacts</span>
        </label>
        <div style={{ fontSize: 11, color: "#9b8e82", marginTop: 8 }}>
          When enabled, every patient added or edited via OP Registration is pushed to Google
          (requires an active connection at that moment).
        </div>
      </div>

      <div style={card}>
        <div style={{ fontWeight: 700, marginBottom: 12 }}>4. Manual sync (existing patients)</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 14 }}>
          <Stat label="Total patients" value={patients.length} />
          <Stat label="Already synced" value={syncedCount} color="#16a34a" />
          <Stat label="Not synced" value={unsyncedCount} color="#d97706" />
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button className="btn" style={{ background: "#16a34a", color: "#fff", border: "none", fontWeight: 700 }}
            disabled={busy || !connected || unsyncedCount === 0}
            onClick={handleSyncUnsynced}>
            ⬆ Sync {unsyncedCount} unsynced
          </button>
          <button className="btn btn-outline" disabled={busy || !connected || patients.length === 0} onClick={handleSyncAll}>
            ↻ Re-sync ALL ({patients.length})
          </button>
          <button className="btn btn-outline btn-sm" disabled={busy} onClick={handleClearMap} style={{ marginLeft: "auto", color: "#991b1b" }}>
            Clear local sync map
          </button>
        </div>
        {progress && (
          <div style={{ marginTop: 14 }}>
            <div style={{ height: 8, background: "#f0ede8", borderRadius: 4, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(progress.done / Math.max(progress.total, 1)) * 100}%`, background: "#16a34a", transition: "width .2s" }} />
            </div>
            <div style={{ fontSize: 12, color: "#6b5e52", marginTop: 6 }}>
              {progress.done} / {progress.total}
              {progress.current?.name ? ` — last: ${progress.current.name}` : ""}
      ...
      {msg && <div style={{ padding: 12, borderRadius: 8, background: msg.includes("fail") || msg.includes("Disconnected") ? "#fee2e2" : "#dcfce7", color: "#1a1714", fontSize: 13 }}>{msg}</div>}
    </div>
  );
}
...
function Stat({ label, value, color }) {
  return (
    <div style={{ background: "#f8f6f2", borderRadius: 10, padding: "12px 14px" }}>
      <div style={{ fontSize: 11, color: "#9b8e82", textTransform: "uppercase", letterSpacing: ".5px" }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 700, color: color || "#1a1714", marginTop: 4 }}>{value}</div>
    </div>
  );
}
...
function InvoiceTemplateEditor({ row, onClose, data: allData }) {
  ...
  const invoiceFileName = () => `Invoice-${(row.id || row.billNo || "bill").replace(/[^\w-]/g, "")}.pdf`;

  const generateInvoicePdf = async () => {
    if (!sheetRef.current) throw new Error("Invoice sheet not ready");
    return nodeToInvoicePdf(sheetRef.current, invoiceFileName());
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      setPdfErr("Could not generate PDF. " + (e?.message || ""));
    } finally {
      setPdfBusy(false);
    }
  };
  ...
  const openWhatsApp = async () => {
    ...
    if (waAttachPdf) {
      ...
      try {
        ...
        setPdfErr("Could not generate the PDF, opening WhatsApp without it. " + (e?.message || ""));
      } finally {
        setPdfBusy(false);
      }
    }
  ...
  return (
    <div className="ss-modal-overlay" style={{ position:"fixed", inset:0, background:"rgba(20,12,6,.55)", zIndex:9999, display:"flex", alignItems:"flex-start", justifyContent:"center", padding:20, overflow:"auto" }} onClick={onClose}>
      ...
      <div className="ss-print-root" onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:14, padding:20, width:"min(1100px, 98vw)", maxHeight:"96vh", overflow:"auto", boxShadow:"0 25px 60px rgba(0,0,0,.35)" }}>
        ...
        <div ref={sheetRef} className="ss-print-sheet" style={{ border:"1px solid #ece6dd", borderRadius:10, padding:tpl === "D" ? 12 : 18, background:"#fffdfa" }}>
          {tpl === "D" ? (
        ...
        <div onClick={() => setShowShare(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.55)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:14, padding:20, width:"min(620px, 96vw)", maxHeight:"92vh", overflow:"auto", boxShadow:"0 25px 60px rgba(0,0,0,.4)" }}>
            ...
            <div style={{ display:"grid", gap:10, fontSize:13 }}>
              ...
              <div style={{ background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:8, padding:10, display:"grid", gap:6 }}>
                <label style={{ display:"flex", alignItems:"center", gap:6, fontWeight:600, color:"#0c4a6e" }}>
                  <input type="checkbox" checked={waAttachPdf} onChange={e => setWaAttachPdf(e.target.checked)} />
                  📎 Attach invoice PDF
                </label>
                <div style={{ fontSize:12, color:"#0c4a6e" }}>
                  WhatsApp links can't attach files automatically — Opening WhatsApp will also download
                  <strong> {invoiceFileName()} </strong>
                  to your device. In the chat that opens, tap the 📎 attach icon and pick that file from your downloads to send it along with the message.
                </div>
                <div>
                  <button className="btn btn-outline btn-sm" disabled={pdfBusy} onClick={handleDownloadPdf}>
                    {pdfBusy ? "Generating…" : "⬇ Download PDF now"}
                  </button>
                </div>
                {pdfErr && <div style={{ color:"#b91c1c", fontSize:12 }}>{pdfErr}</div>}
              </div>
              <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:6 }}>
                <button className="btn btn-outline btn-sm" onClick={() => setWaTemplate(DEFAULT_WA_TEMPLATE)}>Reset template</button>
                <button className="btn btn-sm" disabled={pdfBusy} style={{ background:"#25D366", color:"#fff", fontWeight:700, border:"none", opacity: pdfBusy ? .7 : 1 }} onClick={openWhatsApp}>
                  {pdfBusy ? "Preparing…" : "🟢 Open WhatsApp"}
                </button>
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  return (
    <Shell session={session} onLogout={logout} view={view} setView={setViewNav} can={can} sbStatus={sbStatus} syncing={syncing} lastSync={lastSync} onManualSync={() => syncFromCloud(sbCreds.url, sbCreds.key)}>
      ...
      {view === "gcontacts"    && hasMDAccess(session) && <GoogleContactsSection data={data} />}
...
function Shell({ session, onLogout, view, setView, can, sbStatus, syncing, lastSync, onManualSync, children }) {
  ...
  const NAV = [
    ...
    { id: "gcontacts",   label: "Google Contacts",  icon: "📇", show: hasMDAccess(session) },
...
function PatientsSection({ session, data, mutate, can, audit, onSync, syncing, highlightToday }) {
  ...
  const submit = () => {
    ...
    if (form.id) {
      ...
      try { GContacts.autoSyncPatient(updated); } catch {}
    ...
    try { GContacts.autoSyncPatient(record); } catch {}
